# ╔══════════════════════════════════════════════════════════════╗
# ║     All Otp Bot — Full System Bot                        ║
# ║     Numbers + OTP Forwarding + Full Admin Suite             ║
# ║     Developed by FAYAZ BALOCH (@fayaz786120)                    ║
# ╚══════════════════════════════════════════════════════════════╝
import subprocess
import sys

# Yeh function automatically libraries install kar dega aur upgrade bhi karega
def install_dependencies():
    requirements = ['python-telegram-bot --upgrade', 'requests', 'websockets', 'phonenumbers']
    for package in requirements:
        try:
            clean_name = package.split(' ')[0].replace('-', '_')
            if clean_name == "python_telegram_bot": clean_name = "telegram"
            __import__(clean_name)
            
            if clean_name == "telegram":
                from telegram import CopyTextButton
        except (ImportError, AttributeError):
            print(f"Installing/Updating {package}...")
            try:
                subprocess.check_call([sys.executable, "-m", "pip", "install", *package.split(), "--root-user-action=ignore", "--no-cache-dir"])
...
#  CONFIGURATION
# ═══════════════════════════════════════════════════════════════
BOT_TOKEN      = "8022188232:AAGWuc3-kyoQCvkuTbSr3Lai7ObdeshYbIs"
OWNER_IDS      = [7786910913]
ADMIN_IDS      = [7786910913, 7786910913]
OTP_GROUP_LINK = "https://t.me/+P_K5nqbR4qI0N2Zk"
BOT_NAME       = "ALL OTP GROUP"

REQUIRED_CHANNELS = [
    {"name": "Numbers Channel", "link": "https://t.me/+jdYk61m4aRM0MTZk",      "id": "-1003880733670"},
    {"name": "Chat Group",      "link": "https://t.me/+B9dNaU9S5Bs5Yjlk", "id": "-1003737242686"},
    {"name": "OTP Group",       "link": "https://t.me/+P_K5nqbR4qI0N2Zk",     "id": "-1003866330033"},
]

DEV_CONTACT    = "@fayaz786120"
...
def load_config():
    return load_json(CONFIG_FILE, {
        ...
        "number_bot_link": "https://t.me/+jdYk61m4aRM0MTZk",
        "otp_forward":     True,
        "forward_delay":   0,
        "log_group":       None,
    })
...
def get_otp_keyboard(number: str, otp: str) -> InlineKeyboardMarkup:
    if otp:
        ...
        otp_btn   = InlineKeyboardButton(f"🔑 {fmt_otp}", copy_text=CopyTextButton(text=clean_otp), style="primary")
    else:
        otp_btn   = InlineKeyboardButton("📩 No OTP Detected", callback_data="no_otp", style="danger")
    return InlineKeyboardMarkup([
        [otp_btn],
        [
            InlineKeyboardButton("📞 Numbers",   url="https://t.me/+jdYk61m4aRM0MTZk", style="success"),
            InlineKeyboardButton("💬 Chat", url="https://t.me/+B9dNaU9S5Bs5Yjlk", style="danger")
        ]
    ])
...
async def _otp_sender_task():
    ...
    while True:
        try:
            ...
            if not groups:
                logger.warning("_otp_sender_task: no groups configured!")
                OTP_QUEUE.task_done()
                continue
...
async def send_otp_to_owner(number: str, service: str, otp: str,
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔑 {fmt_otp}", copy_text=CopyTextButton(text=clean_otp), style="success")],
        [
            InlineKeyboardButton("📞 Numbers", url="https://t.me/+jdYk61m4aRM0MTZk", style="success"),
            InlineKeyboardButton("💬 Chat",    url="https://t.me/+B9dNaU9S5Bs5Yjlk", style="danger")
        ]
...
# ═══════════════════════════════════════════════════════════════
#  KEYBOARDS (WITH WORKING STYLES ONLY: "success" or "danger")
# ═══════════════════════════════════════════════════════════════
...
def get_main_menu_keyboard():
    return InlineKeyboardMarkup([
        [b("🧇 𝗚𝗲𝘁 𝗡𝘂𝗺𝗯𝗲𝗿", "show_countries", style="success"),  b("🫁 𝗣𝗿𝗼𝗳𝗶𝗹𝗲", "user_profile", style="success")],
        [b("🧠 𝗗𝗲𝘃𝗲𝗹𝗼𝗽𝗲𝗿", url="https://t.me/fayaz786120", style="success")],
    ])

def get_join_keyboard():
    return InlineKeyboardMarkup([
        [b("📞 Numbers Channel", url="https://t.me/+jdYk61m4aRM0MTZk", style="success")],
        [b("💬 Chat Group",      url="https://t.me/+B9dNaU9S5Bs5Yjlk", style="success")],
        [b("📲 OTP Group",       url="https://t.me/+P_K5nqbR4qI0N2Zk", style="success")],
        [b("✅ I Joined — Check", "check_join", style="success")],
    ])

def get_admin_keyboard(uid: int = 0):
    rows = []
    row = []
    if has_perm(uid, "numbers"): row.append(b("📦 Numbers",    "menu_numbers", style="success"))
    if has_perm(uid, "files"):   row.append(b("📂 Files",      "menu_files", style="success"))
    if row: rows.append(row)
    
    row = []
    if has_perm(uid, "panels"):  row.append(b("📡 Panels",     "menu_panels", style="success"))
    if has_perm(uid, "ivas"):    row.append(b("🔌 IVAS",       "menu_ivas", style="success"))
    if row: rows.append(row)
    
    row = []
    if has_perm(uid, "fetch_sms"):    row.append(b("🔄 Fetch SMS",    "menu_fetch", style="success"))
    if has_perm(uid, "otp_history"):  row.append(b("📋 OTP History",  "menu_otp_history", style="success"))
    if row: rows.append(row)
    
    row = []
    if has_perm(uid, "groups"):    row.append(b("👥 Groups",    "menu_groups", style="success"))
    if has_perm(uid, "broadcast"): row.append(b("📢 Broadcast", "broadcast", style="success"))
    if row: rows.append(row)
    
    row = []
    if has_perm(uid, "settings"):  row.append(b("⚙️ Settings",  "menu_settings", style="success"))
    if has_perm(uid, "advanced"):  row.append(b("🔧 Advanced",  "menu_advanced", style="success"))
    if row: rows.append(row)
    
    if has_perm(uid, "stats"):
        rows.append([b("📊 Stats", "stats", style="success"), b("📡 Status", "status", style="success")])
    rows.append([b("🔄 Refresh", "refresh_admin", style="success"), b("❌ Close", "close_admin", style="danger")])
    return InlineKeyboardMarkup(rows)

def get_numbers_menu():
    return InlineKeyboardMarkup([
        [b("➕ Add Numbers","nb_add_numbers", style="success"),  b("📋 View Stock","nb_number_list", style="success")],
        [b("🗑️ Delete Country","nb_delete_menu", style="danger"),b("📤 Export","nb_export", style="success")],
        [b("📊 Stock Stats","nb_stock_stats", style="success"),  b("🔙 Back","back_to_admin", style="danger")],
    ])

def get_number_manager_keyboard():
    return InlineKeyboardMarkup([
        [b("➕ Add Numbers",    "nb_add_numbers", style="success"),  b("📋 View Stock",   "nb_number_list", style="success")],
        [b("🗑️ Delete Country","nb_delete_menu", style="danger"),   b("📊 Stock Stats",  "nb_stock_stats", style="success")],
    ])

def get_files_menu():
    return InlineKeyboardMarkup([
        [b("📂 List Files","fm_list", style="success"),          b("📥 Download Log","fm_download_log", style="success")],
        [b("📥 Download OTP Store","fm_download_otp", style="success"),b("📥 Download DB","fm_download_db", style="success")],
        [b("🗑️ Clear Log","fm_clear_log", style="danger"),      b("🔙 Back","back_to_admin", style="danger")],
    ])

def get_panels_menu():
    return InlineKeyboardMarkup([
        [b("📋 List Panels","list_panels", style="success"),     b("➕ Add Panel","add_panel", style="success")],
        [b("🗑️ Remove Panel","remove_panel", style="danger"),  b("🧪 Test All","test_panels_menu", style="success")],
        [b("🔄 Fetch Latest","panel_fetch_all", style="success"),b("🔙 Back","back_to_admin", style="danger")],
    ])
...
def get_panels_keyboard(action="view"):
    ...
    for name in panels:
        ...
        style_val = "danger" if action == "remove" else "success"
        keyboard.append([b(label, cb, style=style_val)])
    keyboard.append([b("🔙 Back","menu_panels", style="danger")])
    return InlineKeyboardMarkup(keyboard)

def get_ivas_menu():
    return InlineKeyboardMarkup([
        [b("📋 List IVAS","list_ivas", style="success"),         b("➕ Add IVAS","add_ivas", style="success")],
        [b("🗑️ Remove IVAS","remove_ivas", style="danger"),     b("🔄 Restart All","ivas_restart_all", style="success")],
        [b("🔙 Back","back_to_admin", style="danger")],
    ])
...
def get_ivas_keyboard(action="view"):
    ...
    for name in accounts:
        ...
        style_val = "danger" if action == "remove" else "success"
        keyboard.append([b(label, cb, style=style_val)])
    keyboard.append([b("🔙 Back","menu_ivas", style="danger")])
    return InlineKeyboardMarkup(keyboard)
...
def get_groups_menu():
    ...
    keyboard = []
    for gid in groups:
        keyboard.append([b(f"🗑️ Remove {gid}", f"del_group_{gid}", style="danger")])
    keyboard.append([b("➕ Add Group","add_group_prompt", style="success")])
    keyboard.append([b("📋 Set Log Group","set_log_group", style="success"),
                     b("❌ Clear Log Group","clear_log_group", style="danger")])
    keyboard.append([b("🔙 Back","back_to_admin", style="danger")])
    return InlineKeyboardMarkup(keyboard)

def get_otp_history_menu():
    return InlineKeyboardMarkup([
        [b("📋 Last 10","otp_hist_10", style="success"),         b("📋 Last 20","otp_hist_20", style="success")],
        [b("🔍 Search by Number","otp_search_num", style="success"),b("📤 Export CSV","otp_export_hist", style="success")],
        [b("🗑️ Clear History","otp_clear_hist", style="danger"),b("🔙 Back","back_to_admin", style="danger")],
    ])

def get_fetch_menu():
    return InlineKeyboardMarkup([
        [b("🔄 Fetch All Panels","fetch_all_now", style="success")],
        [b("🔍 Fetch by Number","fetch_by_number", style="success")],
        [b("📡 Fetch Single Panel","fetch_single_panel", style="success")],
        [b("🔙 Back","back_to_admin", style="danger")],
    ])
...
def get_settings_menu():
    ...
    return InlineKeyboardMarkup([
        [b(f"📤 OTP Forward: {fwd}","toggle_otp_forward", style="success" if config.get("otp_forward", True) else "danger")],
        [b(f"⏱ Delay: {delay}s","set_forward_delay", style="success"),
         b("📢 Channel Link","set_channel", style="success")],
        [b("🤖 NumberBot Link","set_numberbot", style="success"),
         b(f"📋 Log Group: {lg}","set_log_group", style="success")],
        [b("🔗 OTP Group Link","set_otp_group_link", style="success"),
         b("👤 Admin Manager","menu_admin_manager", style="success")],
        [b("🔙 Back","back_to_admin", style="danger")],
    ])
...
def get_admin_manager_keyboard():
    ...
    for aid in OWNER_IDS:
        keyboard.append([b(f"👑 {aid}  (Owner)", "noop", style="success")])
    for uid_str, info in staff.items():
        ...
        keyboard.append([
            b(f"🛡️ {name} ({len(perms)} perms)", f"edit_staff_{uid_str}", style="success"),
            b("🗑️", f"remove_staff_{uid_str}", style="danger")
        ])
    keyboard.append([b("➕ Add Staff Member", "add_staff_prompt", style="success")])
    keyboard.append([b("🔙 Back", "menu_settings", style="danger")])
    return InlineKeyboardMarkup(keyboard)
...
def get_staff_perms_keyboard(uid_str: str):
    ...
    for perm_key, perm_label in ALL_PERMISSIONS.items():
        has = perm_key in cur
        icon = "✅" if has else "☑️"
        style_val = "success" if has else "danger"
        keyboard.append([b(f"{icon} {perm_label}", f"toggle_perm_{uid_str}_{perm_key}", style=style_val)])
    keyboard.append([
        b("✅ Grant All",  f"grant_all_{uid_str}", style="success"),
        b("❌ Revoke All", f"revoke_all_{uid_str}", style="danger")
    ])
    keyboard.append([b("🔙 Back", "menu_admin_manager", style="danger")])
    return InlineKeyboardMarkup(keyboard)

def get_advanced_keyboard():
    return InlineKeyboardMarkup([
        [b("🔄 Restart All Workers","restart_workers", style="success"),
         b("🛑 Stop Forwarding","stop_forward", style="danger")],
        [b("▶️ Start Forwarding","start_forward", style="success"),
         b("🔄 Reload Config","reload_config", style="success")],
        [b("🗑️ Clear OTP Store","clear_all_otps", style="danger"),
         b("📤 Export OTP Store","export_otps", style="success")],
        [b("📋 View Logs","view_logs", style="success"),
         b("🔁 Restart Bot","restart_bot", style="danger")],
        [b("🧪 Test All Panels","test_panels_adv", style="success"),
         b("📊 Worker Status","worker_status", style="success")],
        [b("🔙 Back","back_to_admin", style="danger")],
    ])
...
def get_confirmation_keyboard(action: str, extra: str = ""):
    ...
    return InlineKeyboardMarkup([
        [b("✅ Confirm", cd, style="success"), b("❌ Cancel","cancel_action", style="danger")]
    ])

def get_broadcast_keyboard():
    return InlineKeyboardMarkup([
        [b("📝 Text Only","broadcast_text", style="success"),
         b("🔘 With Buttons","broadcast_with_buttons", style="success")],
        [b("🔙 Back","back_to_admin", style="danger")]
    ])
...
def get_countries_keyboard():
    ...
    if not rows:
        return None
    buttons = [b(f"{FLAG_MAP.get(c,'🌍')} {c} ({n})", f"nb_get|{c}", style="success") for c, n in rows]
    kb = [buttons[i:i+2] for i in range(0, len(buttons), 2)]
    kb.append([b("🔄 Refresh","show_countries", style="success")])
    return InlineKeyboardMarkup(kb)

def get_nb_stock_keyboard():
    rows     = db_get_countries()
    keyboard = [[b(f"❌ {c} ({n})  Delete", f"nb_del|{c}", style="danger")] for c, n in rows]
    keyboard.append([b("🔙 Back","menu_numbers", style="danger")])
    return InlineKeyboardMarkup(keyboard)
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    await send_to_all_groups(" ".join(context.args))
    await update.message.reply_text(f"✅ Sent to {len(load_groups())} groups.")
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "public_stats":
        ...
        await query.edit_message_text(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━", parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","back_to_main", style="danger")]]))
        return
    ...
    if data == "user_profile":
        ...
        await query.edit_message_text(
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 𝗕𝗮𝗰𝗸", "back_to_main", style="danger")]]))
        return
        ...
        if phones:
            ...
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [b("🔄 Get 3 More Numbers", f"nb_get|{country}", style="success"),
                     b("🌍 Change Country",      "show_countries", style="success")],
                    [b("📢 OTP Group", url="https://t.me/+P_K5nqbR4qI0N2Zk", style="danger")],
                    [b("🔙 Back",               "back_to_main", style="danger")],
                ]))
        else:
    ...
    if data == "nb_add_numbers":
        ...
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("❌ Cancel","cancel_action", style="danger")]]))
        return
    ...
    if data == "nb_number_list":
        ...
        if not rows:
            await query.edit_message_text("📦 Stock is empty.",
                reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_numbers", style="danger")]]))
            return
    ...
    if data == "fm_list":
        ...
        await query.edit_message_text(
            "📂 <b>FILES:</b>\n<code>" + "\n".join(flist) + "</code>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_files", style="danger")]]))
        return
    ...
    if data.startswith("view_panel_"):
        ...
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","list_panels", style="danger")]]))
        return
    ...
    if data.startswith("confirm_remove_panel_"):
        ...
        await query.edit_message_text(f"✅ Panel <b>{panel}</b> removed.", parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_panels", style="danger")]]))
        return
    ...
    if data == "list_ivas":
        ...
        if not accounts:
            await query.edit_message_text("🟡 No IVAS accounts yet.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
                    [b("➕ Add IVAS","add_ivas", style="success")],
                    [b("🔙 Back","menu_ivas", style="danger")]]))
            return
    ...
    if data == "ivas_restart_all":
        ...
        await query.edit_message_text("✅ All IVAS workers restarted.",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_ivas", style="danger")]]))
        return
    ...
    if data.startswith("view_ivas_"):
        ...
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","list_ivas", style="danger")]]))
        return
    ...
    if data == "fetch_single_panel":
        panels = load_panels()
        kb     = [[b(name.upper(), f"fetch_panel_{name}", style="success")] for name in panels]
        kb.append([b("🔙 Back","menu_fetch", style="danger")])
    ...
    if data.startswith("fetch_panel_"):
        ...
        if result:
            ...
            await query.edit_message_text(
                ...
                f"⏱ {result['time']}",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_fetch", style="danger")]]))
        else:
    ...
    if data in ("otp_hist_10","otp_hist_20"):
        ...
        await query.edit_message_text(text[:4000], parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_otp_history", style="danger")]]))
        return
    ...
    if data.startswith("del_group_"):
        try:
            ...
            await query.edit_message_text(f"✅ Group <code>{gid}</code> removed.", parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_groups", style="danger")]]))
        except:
    ...
    if data == "stats":
        ...
        await query.edit_message_text(
            ...
            f"🗄 OTP Store: <code>{len(load_otp_store())}</code>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━", parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","back_to_admin", style="danger")]]))
        return
    ...
    if data == "confirm_broadcast":
        if uid not in BROADCAST_STATES or BROADCAST_STATES[uid]["step"] != "confirm":
            ...
            return
        await send_to_all_groups(BROADCAST_STATES[uid]["message"])
        del BROADCAST_STATES[uid]
        await query.edit_message_text(f"✅ Sent to {len(load_groups())} groups.",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","back_to_admin", style="danger")]]))
        return
    ...
    if data == "toggle_otp_forward":
        ...
        await query.edit_message_text(f"📤 OTP Forwarding: <b>{st}</b>", parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_settings", style="danger")]]))
        return
    ...
    if data.startswith("grant_all_"):
        ...
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back", "menu_admin_manager", style="danger")]]))
        return
    ...
    if data == "restart_workers":
        ...
        await query.edit_message_text("✅ All workers restarted.",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back","menu_advanced", style="danger")]]))
        return
        ...
        if step == "waiting_group_id":
            ...
            try:
                ...
                await update.message.reply_text(
                    ...
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[b("👤 Open Staff Manager", "menu_admin_manager", style="success")]]))
            except ValueError:
            ...
            if state["type"] == "text":
                ...
                for line in lines:
                    if line.startswith('[') and line.endswith(']') and '|' in line:
                        ...
                        buttons.append(InlineKeyboardButton(parts[0].strip(), url=parts[1].strip(), style="success"))
                    else:
                        msg_text += line + '\n'
...
async def document_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[b("🔙 Back to Admin","back_to_admin", style="danger")]]))
        del NB_STATE[uid]
    except Exception as e:
